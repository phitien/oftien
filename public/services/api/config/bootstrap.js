...
module.exports.bootstrap = async function(done) {
  ...
  const migrationFn = async function() {
    ...
    async function doPatching() {
      ...
      if (patchName) {
        const patch = migrate[patchName];
        try {
          sails.log(`---- ${patchName}: ${patch.description} ----`);
          await patch.fn();
          await doPatching();
        } catch (e) {
          sails.log.warn(`---- ${patchName} ----: Error`, e);
        }
      }
    }
    await doPatching();
    sails.log("Finished migration patches...");
    //end migration
  };
  ...
  if (sails.config.environment === "test") await migrationFn();
